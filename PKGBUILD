...
_extramodules=$(find /usr/lib/modules -type d -iname 6.2.11*xanmod* | rev | cut -d "/" -f1 | rev)
...
build() {
    _kernver=$(find /usr/lib/modules -type d -iname 6.2.11*xanmod* | rev | cut -d "/" -f1 | rev)
    cd "$_pkgname-$_commit"
